...
const riskLikelihoods = new Set<RiskLikelihood>([1, 2, 3, 4, 5]);
const riskImpacts = new Set<RiskImpact>([1, 2, 3, 4, 5]);
const riskStatuses = new Set<RiskStatus>(["open", "mitigated", "accepted", "closed"]);
...
export function validateCreateRiskRequest(body: Record<string, unknown>): CreateRiskRequest | Response {
  ...
  if (!riskLikelihoods.has(body.likelihood as RiskLikelihood)) {
    return apiError(400, "invalid_likelihood", "likelihood must be an integer from 1 to 5.");
  }

  if (!riskImpacts.has(body.impact as RiskImpact)) {
    return apiError(400, "invalid_impact", "impact must be an integer from 1 to 5.");
  }

  if (!riskStatuses.has(body.status as RiskStatus)) {
    return apiError(400, "invalid_status", "status must be open, mitigated, accepted, or closed.");
  }
  ...
  return {
    ...
    status: body.status as RiskStatus,
    owner: owner.trim(),
