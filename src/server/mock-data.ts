...
export function createRisk(request: CreateRiskRequest): Risk {
  const risk: Risk = {
    ...
    severity: deriveSeverity(request.likelihood, request.impact),
    status: "open",
...
function updateConnector(result: SyncResult) {
  ...
  connectorItem.connectionStatus = result.errors.some((error) => error.toLowerCase().includes("unreachable"))
    ? "disconnected"
    : "connected";
  connectorItem.lastSyncStatus = result.status;
  connectorItem.lastSyncedAt = result.finishedAt;
  connectorItem.lastError = result.errors.at(-1) ?? null;
...
function deriveSeverity(likelihood: RiskLikelihood, impact: RiskImpact): RiskSeverity {
  const score = numericRiskValue(likelihood) * numericRiskValue(impact);
...
function numericRiskValue(value: RiskLikelihood | RiskImpact): number {
  if (value === "high") {
    return 3;
  }
